...
- [x] グローバル変数の定義（currentPlant, dyingPlants, etc.）
- [x] setup() の実装（画面サイズ 800x800、60fps、初期化）
- [x] draw() の基本構造（背景、更新、描画の順序）

### Task 3.2: マウス制御の実装

- [x] isMouseMoving() 関数の実装（閾値による判定）
- [x] マウス停止時の処理（新しい Plant 生成）
- [x] マウス移動時の処理（currentPlant の灰化開始、dyingPlants へ移動）

### Task 3.2.1: マウス制御の動作確認

- [x] マウスを止めると種が出現し、枝が伸び始めるか確認
- [x] マウスを動かすと灰化が始まるか確認
- [x] 別の場所で止めると新しい植物が生えるか確認

### Task 3.3: 複数植物の管理

- [x] dyingPlants の更新・描画・削除処理
- [x] 古い植物が灰化中に新しい植物が成長する共存状態の確認
- [x] 完全に消えた植物のリストからの削除

### Task 3.3.1: 複数植物の動作確認

- [x] 灰化中の古い植物と成長中の新しい植物が同時に表示されるか確認
- [x] 複数回繰り返してメモリリークがないか確認
...
- [x] 枝の色（淡い緑）の濃さ・色相の調整
- [x] 花の色（淡いピンク）の濃さ・色相の調整
- [x] 種・粒子の色のバランス調整

### Task 4.2: 速度の微調整

- [x] 成長速度の調整（ゆっくり、瞑想的になっているか）
- [x] 灰化速度の調整（成長より少し速いか）
- [x] 開花速度の調整
...
| Task 3.1 | 2026-02-03 | メインファイル基本構造実装 |
| Task 3.2 | 2026-02-03 | マウス制御実装（静止で生成、移動で灰化） |
| Task 3.2.1 | 2026-02-03 | マウス制御動作確認OK |
| Task 3.3 | 2026-02-03 | 複数植物管理実装 |
| Task 3.3.1 | 2026-02-03 | 複数植物動作確認OK、発芽時間を1-10秒ランダムに変更 |
| Task 4.1 | 2026-02-03 | 色の確認完了、濃い緑(80,200,80)を採用 |
| Task 4.2 | 2026-02-03 | 速度は実装中に調整済み |
| Task 4.3 | 2026-02-03 | 粒子は実装中に調整済み |
| Task 4.4 | 2026-02-03 | グロー効果は実装中に調整済み |
| Task 5.1 | 2026-02-03 | Processing 2.2.1互換性確認完了 |
| Task 5.2 | 2026-02-03 | 最終調整完了、燃える灰エフェクト追加 |
